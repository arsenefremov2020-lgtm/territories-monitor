...
latest_doc = pd.read_sql(text(documents_query), engine)
...
if not latest_doc.empty:
    doc = latest_doc.iloc[0]

    st.info(
        f"Джерело даних: {doc['source_name']} | "
        f"Наказ №{doc['document_number']} | "
        f"Останнє оновлення бази: {doc['loaded_at']}"
    )

    st.markdown(f"[Відкрити джерело]({doc['source_url']})")
